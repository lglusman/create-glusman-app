...
namespace WebApi.Models.Helper
{
    ...
    {
        public static IQueryable<T> AddQueryIncludes<T>(IQueryable<T> query, string includes, DbContexto context) where T : class
        {
        ...
        internal static IQueryable<T> AddQueryIncludes<T>(IQueryable<T> query, string includes, int cantidad, int pagina, string order, DbContexto context) where T : class
        {
            var efProperties = context.Model.FindEntityType(typeof(T))?.GetNavigations();
